...
extension StringExtensions on String {
  ...
  /// Exemplo de uso:
  /// ```dart
  /// String result = 'Café'.removeDiacriticsToLowerCase;
  /// print(result); // Saída: 'cafe'
  /// ```
  String get removeDiacriticsToLowerCase {
    if (isNotEmpty) {
  ...
  String get formatBrDocument {
    if (isNotEmpty) {
      String cleanedDocument = replaceAll(RegExp(r'[^0-9Xx]'), '');
  ...
  String? canBePlural(int quantityToCount) =>
  ...
  String get prepareCurrencyForParsing {
    if (isNotEmpty) {
      ...
      if (firstIndexNumber != -1) {
        String preparedValue = substring(firstIndexNumber)
            .replaceAll('.', '')
            .replaceAll(',', '.');
  ...
  String get removeSpecialCharacters {
    if (isNotEmpty) {
      return replaceAll(RegExp(r'[^\w\s]|_'), '');
    }
    return this;
  }
}
